...
file_count = 0
total = 0
output_dir = 'datasets/UIT/'

for file_name in file_names:
    file_directory = f'datasets/UIT/fixed_test.json'
    ...
    for id, content in data.items():
        ...
        if id not in DIC:
            DIC[id] = []  # Initialize an empty list for the ID if it doesn't exist
        for key, value in content.items():
            if key == 'source':
                for i, sentence in value.items():
                    for sen in sentence: 
                        words = re.split(r'[^a-zA-Z0-9áàảãạăắằẳẵặâấầẩẫậđéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵÁÀẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬĐÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴ]+', sen)  # Tách dựa trên các ký tự không phải chữ cái hoặc số  # Loại bỏ các phần tử rỗng nếu có  # Split the sentence into words
                        for word in words:  # Check each word individually
                            if not is_Vietnamese(word)[0]:
                                DIC[id].append(word)  # Add the non-Vietnamese word to DIC

for id, words in DIC.items():
    DIC[id] = list(set(words))  # Remove duplicates

output_file = 'sua/UIT-ViNewsOnSum-dataset/DIC_output1.json'
