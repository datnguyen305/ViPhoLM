...
file_count = 0
total = 0
output_dir = '/datasets/UIT-ViNewsOnSum-dataset/'

for file_name in file_names:
    file_directory = f'/datasets/UIT-ViNewsOnSum-dataset/{file_name}.json'
        ...
        for key, value in content.items():
            if key == 'source':
                for i, sentence in value.items():
                    for sen in sentence: 
                        words = re.split(r'[,\.\s]+', sen)  # Split the sentence into words
...
output_file = 'sua/UIT-ViNewsOnSum-dataset/DIC_output.json'
