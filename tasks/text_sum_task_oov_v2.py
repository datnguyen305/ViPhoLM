...
@META_TASK.register()
class TextSumTaskOOV_v2(BaseTask):
    ...
    def train(self):
        ...
        with tqdm(desc='Epoch %d - Training' % (self.epoch+1), unit='it', total=len(self.train_dataloader)) as pbar:
            for it, items in enumerate(self.train_dataloader):
                # Unpack dữ liệu từ InstanceList
                input_ids = items.input_ids.to(self.device)          # (B, S, W)
                pos_ids = items.pos_ids.to(self.device)              # (B, S, W)
                ner_ids = items.ner_ids.to(self.device)              # (B, S, W)
                tfidf_ids = items.tfidf_ids.to(self.device)          # (B, S, W)
                ...
                _, loss = self.model(
                    ...
                    labels, extra_zeros, extended_source_idx
                )
    ...
    def evaluate_metrics(self, dataloader: DataLoader) -> dict:
        ...
        with tqdm(desc='Epoch %d - Evaluating' % (self.epoch+1), unit='it', total=len(dataloader)) as pbar:
            for items in dataloader:
                ...
                pos_ids = items.pos_ids.to(self.device)
                ner_ids = items.ner_ids.to(self.device)
                tfidf_ids = items.tfidf_ids.to(self.device)
                ...
                with torch.no_grad():
                    # Hàm predict/generate cũng cần các features phân cấp
                    prediction_indices = self.model.predict(
                        input_ids, pos_ids, ner_ids, tfidf_ids,
                        extended_source_idx, extra_zeros
                    )

                    decoded_preds = self.decode_pgn_output(prediction_indices, oov_list_batch)
                    decoded_labels = self.decode_pgn_output(label, oov_list_batch)

                    # items.id[0] vì batch_size=1 khi eval
                    sample_id = items.id[0]
                    gens[sample_id] = decoded_preds[0]
                    gts[sample_id] = decoded_labels[0]
