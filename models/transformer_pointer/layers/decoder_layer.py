...
import torch
from models.transformer_pointer.layers.positionwise_feed_forward import PositionwiseFeedForward
from models.transformer_pointer.utils.clone import clones
from models.transformer_pointer.layers.sub_layer_connection import SublayerConnection
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, memory, tgt_causal_mask, tgt_padding_mask, memory_padding_mask):
        ...
        # x, attn_weights = self.sublayer[1](x, lambda x: self.cross_attn(query=x, 
        #                                                  key=memory, 
        #                                                  value=memory, 
        #                                                  key_padding_mask=memory_padding_mask,
        #                                                  need_weights = True))
        
        # My own fucking sub_layer
        # x: (B, S, H)
        nx = self.norm(x)
        x_processed, attn_weights = self.cross_attn(
            query = nx, 
            key = memory,
            value = memory,
            key_padding_mask = memory_padding_mask,
