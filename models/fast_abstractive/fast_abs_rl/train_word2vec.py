...
class Sentences(object):
    ...
    def __iter__(self):
        for i in range(self._n_data):
            ...
            for s in concatv(data['source'], data['target']):
...
def main(args):
    ...
    model = gensim.models.Word2Vec(
        vector_size=args.dim, min_count=5, workers=16, sg=1)
    ...
    model.save(join(save_dir, 'word2vec.{}d.{}k.bin'.format(
        args.dim, len(model.wv.key_to_index)//1000)))
    model.wv.save_word2vec_format(join(
        save_dir,
        'word2vec.{}d.{}k.w2v'.format(args.dim, len(model.wv.key_to_index)//1000)
    ))
