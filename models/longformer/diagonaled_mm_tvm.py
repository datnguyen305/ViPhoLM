...
class DiagonaledMM(torch.autograd.Function):
    ...
    def _compile_function(dtype: str, device: str, b0: int = 4, b1: int = 4, b2: int = 16):
        ...
        # Fix: Properly set target and target_host
        if device == 'cuda':
        ...
        # Only apply GPU-specific optimizations for CUDA target
        if device == 'cuda':
    ...
    def _load_compiled_function(dtype: str, device: str):
        from tvm.runtime.module import load_module as load  # this can be the small runtime python library, and doesn't need to be the whole thing
